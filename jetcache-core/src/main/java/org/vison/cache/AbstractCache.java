...
import org.vison.cache.event.CacheGetAllEvent;
import org.vison.cache.event.CacheGetEvent;
...
import org.vison.cache.support.JetCacheExecutor;
import org.vison.cache.support.SquashedLogger;
...
public abstract class AbstractCache<K, V> implements Cache<K, V> {
    ...
    /**
     * 如果缓存中没有对应的值,则使用loader加载值并返回,并将值更新到缓存中.
     * 如果loader返回null,则根据cacheNullWhenLoaderReturnNull参数判断是否缓存null值.
     * 如果expireAfterWrite参数大于0,则设置缓存过期时间.
     * @param key
     * @param loader
    ...
    static <K, V> V computeIfAbsentImpl(K key, Function<K, V> loader, boolean cacheNullWhenLoaderReturnNull,
        ...
        if (r.isSuccess()) {
            // 缓存中存在值,则直接返回
            logger.debug("命中缓存,key:{} ,value:{} ,cacheType:{}",key,r.getValue(),cacheType);
            return r.getValue();
        } else {
            ...
            logger.debug("缓存未命中,从方法中获取,key:{} ,value:{} ,cacheType:{}",key,loadedValue,cacheType);
            return loadedValue;
        }
    }
